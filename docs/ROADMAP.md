...
| Phase 2 领域建模 + 常规电商闭环 | ✅ 已完成 | 2026-09-13 | 2.1 ✅ 2.2 ✅ 2.3 ✅（含修掉真实 CSRF 缺陷）2.4 ✅（库存双策略 + 并发不超卖）2.5 ✅（订单状态机 + 明细快照 + PO/DTO/VO 分层）2.6 ✅（6 条高频 SQL 的 EXPLAIN 前后对比，6 个索引，含一条「刻意不加」的实测论证）—— **Phase 2 全部完成** |
| Phase 3 Redis 缓存 + 分布式锁 | ✅ 已完成 | 2026-09-13 | 3.1 ✅ 序列化（含两处修正：JavaTimeModule、自定义 mapper 必须自己 activateDefaultTyping）3.2 ✅ 商品详情 Cache-Aside（专用缓存 VO）3.3 ✅ 分类缓存 3.4 ✅ 穿透（布隆过滤器 + 空值哨兵）3.5 ✅ 击穿（互斥锁重建）3.6 ✅ 雪崩（TTL 抖动）3.7 ✅ 一致性（先更库再删缓存 + 延迟双删）3.8 ✅ Redisson 锁（重复下单）3.9 ✅ 命中率 98.1% / 命中 778µs vs 直查库 1816µs —— **代码与验证已完成，尚未 commit**（详见文末 Phase 3 报告） |
...
| 2.4 库存 + 乐观锁/悲观锁 | ✅ | 策略模式两种实现（`shop.stock.strategy` 切换）；8 并发抢同一件商品实测**恰好 1 单、零超卖**；
...
**🐞 2.4 期间修掉的第二个问题：预期内的业务失败被记成 HTTP 500**

并发测试断言「失败方拿到的不是 500」时发现：`ErrorCode.BIZ_ERROR` 的 code 本身就是 `500`，
而 `toHttpStatus` 只映射 400/401/403/404，其余一律落到 500 ——
于是「商品已被别人买走」这种**预期内的抢购失败**在监控里与真正的服务端故障无法区分，
错误率与告警全部失真。已新增 `CONFLICT(409)` 并映射到 `HttpStatus.CONFLICT`：
语义上正是「请求与资源的当前状态冲突」，对应乐观锁/条件更新失败这一类场景。
（这个缺陷同样是「单次操作测不出来」的类型：只有并发抢购才会大量产生它。）

**乐观锁 vs 悲观锁：两种实现与实测结论**

| | 乐观锁（默认） | 悲观锁 |
|---|---|---|
| 手段 | 读版本 → 带版本条件 UPDATE → 影响行数为 0 即冲突 | `SELECT ... FOR UPDATE` 先锁行再操作 |
| 冲突时 | 重试（`shop.stock.max-retry`，默认 3） | 阻塞等待锁，不重试 |
| 适用 | 冲突概率低（绝大多数业务） | 冲突概率高且临界区极短 |
| 代价 | 冲突激烈时重试会放大数据库压力 | 持锁期间其他请求排队，超时抛 `LockWaitTimeoutException` |
| 实测（8 并发抢 1 件） | **恰好 1 单成功、7 个 409、库存归零、零超卖** | **同上** |

> 关于「事务内重试是空转」这一条，是本阶段最值得讲的细节：MySQL 默认 REPEATABLE READ，
> 事务内第一次快照读之后后续普通 SELECT 复用同一份快照，所以「读版本→失败→再读版本」拿到的还是旧版本，
> 重试必然再次失败。因此重试必须在**事务外层**、每轮开一个新事务（`PurchaseService` 不加 `@Transactional`，
> 通过代理调用 `PurchaseTxService.doPurchase`）。这也顺带解释了 UPDATE（当前读）与 SELECT（快照读）的区别。

**2.5 订单状态机与明细快照**

重构前的问题（三个，都能在面试里展开）：

1. **状态流转是 Controller 里的 if/else**：「等待发货→已发货→订单已完成」硬编码，
   没有「允许的流转」定义，也没有非法流转校验（「已取消 → 已发货」拦不住），新增状态要改多处；
2. **用中文字符串当状态码**：不可枚举、不可校验，写错一个字就落库成脏数据；
3. **没有并发保护**：重复点击「发货」会推进两次。

改法：`OrderStatus` 枚举 + **显式流转表**（静态 Map，本身就是业务规则的可读文档），
并用「带旧状态条件的 UPDATE」把校验下沉到数据库：

```sql
update lxy_order set status = #{to}, `condition` = #{legacy}
 where id = #{id} and status = #{from}     -- 影响行数 0 = 状态已变，拒绝本次操作
```

一条 SQL 同时解决「非法流转」与「并发重复推进」：两个请求都想从 PAID 出发时，只有一个能匹配到它。

**订单明细存快照，而不是展示时 JOIN 商品表**

`order_item` 存的是下单瞬间的商品名与价格。订单是**历史凭证**：卖家之后改名或改价，
历史订单必须仍显示成交当时的信息。原实现展示订单时 JOIN `lxy_product`，商品一改历史订单就跟着变 ——
电商建模的经典错误。金额用 `BigDecimal`（DB 侧 `decimal(10,2)`）而非 `double`：
二进制浮点无法精确表示 0.1 这类十进制小数，累加会出现 0.30000000000000004。

**PO / DTO / VO 分层**（本次顺带落地，面试常问）

| 类型 | 本项目的类 | 职责 |
|---|---|---|
| PO | `OrderPO`、`ProductStock`、`OrderItem` | 与表结构一一对应，只在持久层使用 |
| DTO | `CartItemRequest` | 接口入参，带参数校验注解 |
| VO | `Order`、`Product` | 页面/接口展示，字段靠 JOIN + 别名拼装 |

收益很具体：改表结构只改 PO，改接口契约只改 DTO/VO。
`Order` 之前既当展示模型又当写模型，导致「要写哪些列」不清晰。

**验证证据（Phase 2 套件 H 段，20 项）**：下单后 `order_no` 匹配 `SO+17位时间戳+4位随机`、全表唯一、
`status=PAID` 且旧文案双写、`total_amount` 取到商品价、`pay_time` 已写、明细快照的商品名/价格/数量/卖家全部匹配；
卖家登录后 `PAID→SHIPPED→COMPLETED` 逐步推进且旧文案同步；终态再推进返回 409 并说明原因；
3 个并发推进满足「不越级 + 成功次数=推进步数」两个不变式。

**测试脚手架的假失败清单（Phase 2.4–2.5 积累，每条都浪费过时间）**

| 症状 | 真因 |
|---|---|
| 8 个并发请求只收到 2 条返回码 | 多进程同时 `>>` 追加同一文件，Windows/MSYS 下丢写 → 各写各的文件再合并 |
| 脚本在并发测试处死等超时 | `wait` 不带参数会连 `app_start` 起的 java 进程一起等 → 只 `wait` 具体 PID |
| 「失败方拿到 400」断言失败 | 断言与实现语义未对齐：抢购失败应当是 409，不是 400/500 |
| 「并发推进只有 1 次成功」断言失败 | **断言本身错了**：接口语义是「推进一格」，3 个并发成功 2 次完全合法 → 改为断言真正的不变式（不越级 + 成功次数=步数） |
| 「单测数不符」误报 | 断言把数量写死成 55，新增 10 个单测就误报 → 断言只盯 `Failures: 0/Errors: 0`，数量动态读取 |
| 套件 B 段报「明细覆盖订单数 31/28」 | **真实漏洞**：并发测试的清理只删了 `lxy_order`，没删 `order_item`，留下孤儿明细 —— 是断言把它抓出来的 |
...
✅ 已完成  Phase 1  Boot 3 升级 + Security6/JWT/RBAC/BCrypt/CSRF
                    ...
                    + 自定义 starter（考点 16）
✅ 已完成  Phase 2  领域建模（schema_v2.sql）+ MyBatis-Plus + cart_item 关联表
                    + 库存双策略（乐观锁/悲观锁）+ 订单状态机/明细快照/PO-DTO-VO
                    + 索引优化（6 条高频 SQL 的 EXPLAIN 前后对比）
✅ 已完成  Phase 3  Redis 缓存 + 分布式锁（3.1–3.9 全部完成，见文末 Phase 3 报告）
          验证    单测 96 个（应用）+ 11 个（starter）全绿
                   端到端 76 项断言（.hermes/verify-phase3.sh）全绿
                   命中率 98.1% / 命中 778µs vs 直查库 1816µs / 20 并发只查 1 次库
📦 数据备份  .hermes/backups/shop-phase1-<时间戳>.sql（8 表全量，已校验）

▶ 状态（2026-09-13 晚 · 用户喊停）
  Phase 3 的代码、测试、文档都已完成并验证通过，但**尚未 git commit**（工作区有 18 个变更文件）。
  下次开工第一件事：先提交，再进 Phase 4。

▶ 下次下一步
  1) git commit（Phase 3 全部改动；建议按 3.1 修正 / 3.2–3.6 缓存主体 / 3.7 一致性 / 3.8 锁 / 3.9+文档 分组）
  2) Phase 4（必做）：Docker 起 RabbitMQ → 声明交换机/队列/死信队列 → 异步下单 → 订单超时自动取消
     （延迟消息 + @Scheduled 扫表兜底）→ 消费幂等（唯一索引）→ 死信重试 → 秒杀基础版（Redis+Lua 预减库存）

完成标志（Phase 3）：商品详情给出「命中缓存 vs 直查库」的实测耗时，缓存键在 redis-cli 里可读 ✅ 已达成
```
...
| 3 | 基于 Redis 实现商品缓存与「穿透 / 击穿 / 雪崩」治理，命中率 **98.1%**；Redisson 分布式锁解决重复下单（同一用户 8 并发 → 仅 1 笔订单） | 命中缓存 **778µs** vs 直查库 **1816µs**（2.3 倍；该 SQL 本身是主键 JOIN，已说明倍数为何不大）|
...
# Phase 3 完成报告（2026-09-13，Redis 缓存 + 分布式锁）

## 交付内容

| Task | 内容 | 状态 |
|---|---|---|
| 3.1 | `RedisConfig`：key=String / value=Jackson JSON；**修掉两处序列化坑**（见下）| ✅ |
| 3.2 | 商品详情 Cache-Aside（`ProductCacheService`）+ `ProductCacheVO` + 浏览数原子自增 | ✅ |
| 3.3 | 分类列表缓存（`CategoryService`，读多写少）| ✅ |
| 3.4 | 穿透：`RBloomFilter`（启动全量装载 + 新增告知）+ 空值哨兵 `__NULL__`（60s）| ✅ |
| 3.5 | 击穿：`setIfAbsent` 重建锁 + 双重检查 + 400ms 等待后降级查库 | ✅ |
| 3.6 | 雪崩：TTL = 基础值 + 随机抖动（`CacheTtl.withJitter`）| ✅ |
| 3.7 | 一致性：先更库再删缓存 + 延迟双删（`CacheInvalidator`，500ms）| ✅ |
| 3.8 | Redisson `RLock` 挡「同一用户重复下单」（等待 0，租期 -1 走看门狗）| ✅ |
| 3.9 | 命中率与耗时实测（`/shop/api/cache/stats` + Redis `INFO stats`）| ✅ |
| 验证 | `.hermes/verify-phase3.sh`：**76 项断言全绿**；单测 96（应用）+ 11（starter）全绿 | ✅ |
...
| 命中缓存 vs 直查库 | **778 µs vs 1816 µs（2.3 倍）** —— 说明：该 SQL 本身是主键 JOIN，只有 1.8ms，收益有限；倍数不大要说清原因 |
| 单 key 冷启动 + 20 并发 | 只有 **1 次**查库；命中 19 / 未命中 1（合计 20，口径不重复计数）|
| 空值缓存 | 10 次无效 id 请求 **0 次**查库 |
| 布隆过滤器 | 不存在的 id（超出现有 id 空间）→ **不查 Redis 也不查库**；装载 3659 个 id 耗时 1.3s |
| 分布式锁 | 同一用户 8 并发下单 → 成功 **1** 笔、其余 409、订单数 **1** |
| 降级 | Redis 不可达 / Redisson 关闭 → 应用**照常启动**、接口 200、下单成功（日志可证）|

## 本阶段踩的坑（都是真实排错过程，面试素材）

| 坑 | 现象 | 处理 |
|---|---|---|
| 默认 ObjectMapper 不支持 JDK8 时间 | 3.3 首次缓存 `Category` 时抛 `InvalidDefinitionException`（`LocalDateTime`）| 注入容器里的 `ObjectMapper`（已注册 JavaTimeModule）的 `copy()` 给序列化器 |
| 传自定义 mapper 会丢掉 `@class` | 值写进去了、读回来是 `LinkedHashMap`，服务里 `(Product) cached` 直接 ClassCastException | 自己 `activateDefaultTyping`；把「生产与测试共用同一个构造方法」写进设计，避免测试测的是另一套配置 |
| **直接缓存实体是有损的** | `java.sql.Date` 经 Jackson 往返：时分秒丢失 + 时刻被重解释（UTC+8 上偏 8 小时）| 改用缓存专用 VO，日期用字符串承载（`2024-05-06`），两侧完全对称 |
| 统计口径把命中率算成一半 | 「先记未命中、重建后再记命中」→ 一个请求同时进分子和分母（20 个请求 19 个命中，报表显示 48%）| 一次请求只记一种最终结果，并加断言「命中+未命中 == 请求数」 |
| **curl 是 Windows 程序，不认 MSYS 路径** | `curl -o /c/Users/...` 静默写失败 → 所有依赖返回体的断言全红（本地铁律里写过，仍然踩了）| `curl` 的文件参数（`-o` / `-b` / `-c`）一律相对路径（先 `cd $W`）；MSYS 工具（grep/cat）用绝对路径没问题 |
| 用 `grep 'xac'` 判断 JDK 序列化 | 中文的 UTF-8 字节里就有 `0xAC`（「马」= e9 a9 ac）→ 假阳性 | 判据改成「值是否以 `{` 开头」 |
| 「7 个请求都该被锁挡下」的错误预期 | 实际只有 1 个被锁挡、其余在库存条件更新处失败 | 等待时间 0 只挡「时间重叠」的请求；两种 409 都正确，断言改为「至少 1 个被锁挡」|
| `taskkill //F` 无效 | `MSYS_NO_PATHCONV=1` 下 `//F` 被当成非法参数 | 用单斜杠 `/F /PID` |
| 布隆过滤器的已知限制 | 外部直接写库（不经应用）的商品会被误判为不存在 | 已记录为**已知限制**而非 bug：写入路径必须可控 + 启动全量重载自愈（脚本里实测了「重启后恢复」）|

## 未覆盖（诚实说明）

- 没有做 **压测**（QPS / P99）—— 这是 Phase 6 的内容；本阶段的性能数字都是应用侧埋点平均值。
- 没有验证 **多实例部署**：分布式锁与击穿锁在单实例下测的，多实例下的竞争行为未实测。
- 延迟双删只压低了不一致窗口，**没有做 binlog 级强一致**（`Canal`），已在文档里写明取舍。
- Phase 3 的改动**尚未 git commit**（用户中途喊停），下次开工先提交。
